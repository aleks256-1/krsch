...
void initCreature(struct creature ** creature){
	*creature = (struct creature*)malloc(sizeof(struct creature));
    ...
    (*creature)->cells[0].v[1] = (*creature)->cells[0].dv[1] = 255;

	(*creature)->cells[1].v[0] = (*creature)->cells[1].dv[0] = 255;
	(*creature)->cells[1].v[1] = (*creature)->cells[1].dv[1] = 255;

	(*creature)->cells[2].v[0] = (*creature)->cells[2].dv[0] = 0;
    (*creature)->cells[2].v[1] = (*creature)->cells[2].dv[1] = 255; 

	(*creature)->cells[3].v[0] = (*creature)->cells[3].dv[0] = 0;
	(*creature)->cells[3].v[1] = (*creature)->cells[3].dv[1] = 255;     

	(*creature)->cells[4].v[0] = (*creature)->cells[4].dv[0] = 255;
	(*creature)->cells[4].v[1] = (*creature)->cells[4].dv[1] = 255;

	(*creature)->cells[5].v[0] = (*creature)->cells[5].dv[0] = 255;
	(*creature)->cells[5].v[1] = (*creature)->cells[5].dv[1] = 255;

	(*creature)->cells[6].v[0] = (*creature)->cells[6].dv[0] = 0;
    (*creature)->cells[6].v[1] = (*creature)->cells[6].dv[1] = 255;

	(*creature)->cells[7].v[0] = (*creature)->cells[7].dv[0] = 0;
	(*creature)->cells[7].v[1] = (*creature)->cells[7].dv[1] = 255;

	(*creature)->cells[8].v[0] = (*creature)->cells[8].dv[0] = 255;
    (*creature)->cells[8].v[1] = (*creature)->cells[8].dv[1] = 0;

	(*creature)->cells[9].v[0] = (*creature)->cells[9].dv[0] = 255;
	(*creature)->cells[9].v[1] = (*creature)->cells[9].dv[1] = 0;

    (*creature)->cells[10].v[0] = (*creature)->cells[10].dv[0] = 0;
    (*creature)->cells[10].v[1] = (*creature)->cells[10].dv[1] = 0;

	(*creature)->cells[11].v[0] = (*creature)->cells[11].dv[0] = 0;
	(*creature)->cells[11].v[1] = (*creature)->cells[11].dv[1] = 0;

	(*creature)->cells[12].v[0] = (*creature)->cells[12].dv[0] = 255;
	(*creature)->cells[12].v[1] = (*creature)->cells[12].dv[1] = 0;

	(*creature)->cells[13].v[0] = (*creature)->cells[13].dv[0] = 255;
	(*creature)->cells[13].v[1] = (*creature)->cells[13].dv[1] = 0;

	(*creature)->cells[14].v[0] = (*creature)->cells[14].dv[0] = 0;
	(*creature)->cells[14].v[1] = (*creature)->cells[14].dv[1] = 0;

	(*creature)->cells[15].v[0] = (*creature)->cells[15].dv[0] = 0;
	(*creature)->cells[15].v[1] = (*creature)->cells[15].dv[1] = 0;
}
...
void load(struct genome * genome) {
    ...
    genome->genes = (struct gene*)malloc(genome->length * sizeof(struct gene));
    for (i = 0; i < genome->length; i++) {
        genome->genes[i].cond_length = 1;
        genome->genes[i].oper_length = 1;
        genome->genes[i].cond = (struct cond*)malloc(genome->genes[i].cond_length * sizeof(struct cond));
		genome->genes[i].operons = (struct operon*)malloc(genome->genes[i].oper_length * sizeof(struct operon));
    }
    ...
    while(fgets(line, sizeof(line), fp)) {
        ...
        cout << (int)genome->genes[current_gene].cond[current_cond_oper].substance << " " << (int)genome->genes[current_gene].cond[current_cond_oper].sign << " " 
            << (int)genome->genes[current_gene].cond[current_cond_oper].threshold << " " << (int)genome->genes[current_gene].operons[current_cond_oper].substance << " " 
            << (int)genome->genes[current_gene].operons[current_cond_oper].sign << " " << (int)genome->genes[current_gene].operons[current_cond_oper].rate << "\n"; 
        current_gene++;
    }
    ...
    for(int i = 0; i < countOfCells; i++){ 
        for (int j = 0; j < genome->length; j++) {
            ...
            for (int k = 0; k < genome->genes[j].cond_length; k++) {
                if (genome->genes[j].cond[k].sign == 1) {
                    // >
                    delta[k] = creature->cells[i].v[genome->genes[j].cond[k].substance] - genome->genes[j].cond[k].threshold;
                    cout << "cell#"<<i<<" sign = "<< 1 << " substance in " << (int)genome->genes[j].cond[k].substance << 
                        " = " << (float)creature->cells[i].v[genome->genes[j].cond[k].substance] << " thereshold = " << 
                        (int)genome->genes[j].cond[k].threshold << " delta = " << delta[k] << '\n';
                } 
                if (genome->genes[j].cond[k].sign == 0) {
                    // <
                    delta[k] = genome->genes[j].cond[k].threshold - creature->cells[i].v[genome->genes[j].cond[k].substance];
                    cout << "cell#"<<i<<" sign = "<< 0 << " substance in " << (int)genome->genes[j].cond[k].substance << 
                        " = " << (float)creature->cells[i].v[genome->genes[j].cond[k].substance] << " thereshold = " << 
                        (int)genome->genes[j].cond[k].threshold << " delta = " << delta[k] << '\n';
                }
            }
            ...
            for (int k = 0; k < genome->genes[j].oper_length; k++) {
                for (int p = 0; p < genome->genes[j].cond_length; p++) {
                    if (genome->genes[j].operons[k].sign == 1) { 
                        // - 
                        cout << "cell#"<<i<<" sign = "<< 1 << " substance in " << (int)genome->genes[j].operons[k].substance << 
                            " = " << (float)creature->cells[i].dv[genome->genes[j].operons[k].substance] << " rate = " << 
                            (int)genome->genes[j].operons[k].rate << " delta = " << delta[p] << " sigma(delta[p]) = "<< calc_sigma(delta[p]/127) << '\n';                  
                        ...
                        // +
                        cout << "cell#"<<i<<" sign = "<< 0 << " substance in " << (int)genome->genes[j].operons[k].substance << 
                            " = " << (float)creature->cells[i].dv[genome->genes[j].operons[k].substance] << " rate = " << 
                            (int)genome->genes[j].operons[k].rate << " delta = " << delta[p] << " sigma(delta[p]) = "<< calc_sigma(delta[p]/127) << '\n';
